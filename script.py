...
#logging object configuration (root logger) - writes errors to app.log
...
''' fetches coin toss by [0,1] random result based on random.org API 
   unless network error/connection unavailable '''
def toss_coin():
...
''' fetches dice roll from random.org API with a modified payload
   unless network error/connection unavailable '''   
def roll_die():
...
def play_guess_game():
    ...
    total_guesses = 0
    replay = True
    #Stores result out comes (Mostly for the cointoss game)
